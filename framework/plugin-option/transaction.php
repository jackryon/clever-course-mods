...
<?php
	...
	for($i=($record_per_page*($current_page - 1)); $i<$record_num && $i<($record_per_page * $current_page); $i++){ $result = $results[$i];
		...
		// mods mods mods
		$student_first_name = $student_info ? $student_info->first_name : "noname";
		$student_last_name = $student_info ? $student_info->last_name : "";
...
<?php
	...
	if( $max_num_page > 1 ){
		...
		if($current_page > 1){
			...
			echo '<a class="prev page-numbers" href="' . add_query_arg($page_var) . '" >';
			echo __('&lsaquo; Previous', 'gdlr-lms') . '</a>';
		}
		for($i=1; $i<=$max_num_page; $i++){
			$page_var['paged'] = $i;
			if( $i == $current_page ){
				echo '<span class="page-numbers current" href="' . add_query_arg($page_var) . '" >' . $i . '</span>';
			}else{
				echo '<a class="page-numbers" href="' . add_query_arg($page_var) . '" >' . $i . '</a>';
			}
		}
		if($current_page < $max_num_page){
			$page_var['paged'] = intval($current_page) + 1;
			echo '<a class="next page-numbers" href="' . add_query_arg($page_var) . '" >';
